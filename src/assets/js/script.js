...
class AdminIntegration {
    ...
    listenForContentUpdates() {
        ...
        // Listen for blog posts updates specifically
        window.addEventListener('blogPostsUpdated', (e) => {
            console.log('Blog posts updated from admin panel:', e.detail);
            // Refresh the blog section with new posts
            this.refreshBlogSection();
            this.showUpdateNotification('Blog posts updated!');
        });
    }
    ...
    refreshBlogSection() {
        // Reload blog posts from localStorage
        const blogData = localStorage.getItem('shamrat_blog_posts');
        if (blogData) {
            try {
                    ...
                    blogGrid.innerHTML = posts.map((post, index) => `
                        <article class="blog-card">
                            ...
                            <div class="blog-content">
                                ...
                                <h3>${post.title}</h3>
                                <p>${post.excerpt}</p>
                                <a href="blog-post/${post.title.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '')}-${index}.html" class="blog-link">Read More <i class="fas fa-arrow-right"></i></a>
                            </div>
                        </article>
                    `).join('');
                }
            } catch (e) {
                console.error('Error refreshing blog section:', e);
            }
        }
    }
    ...
    showUpdateNotification(customMessage = 'Content updated from admin panel') {
